...
def exportUserListCsv(request):
    ...
    response['Content-Disposition'] = 'attachement; filename=UserList' + \
        str(datetime.now())+'.csv'
    writer = csv.writer(response)
    writer.writerow(['names', 'email', 'phone Number', 'Address', 'Gender'])
    users = User.objects.all()
    for user in users:
        writer.writerow([user.names, user.email, user.phone,
...
def exportUserListPdf(request):
    ...
    response['Content-Disposition'] = 'inline; attachement; filename=UserList' + \
...
def exportUserListExcel(request):
    ...
    rows = User.objects.all().values_list(
        'names', 'email', 'phone', 'address', 'gender')
    for row in rows:
        row_num += 1
